...
Relaciones (Associations): El mecanismo que permite conectar diferentes modelos/tablas entre sí, reflejando la lógica del negocio.

Uno a Muchos (One-to-Many): Un tipo de relación donde un registro de una tabla (el "uno", ej: Usuario) puede estar vinculado a múltiples registros de otra tabla (los "muchos", ej: Publicacion).

hasMany(OtroModelo): Método de Sequelize que se utiliza en el modelo "uno" para establecer la relación. Sequelize añade métodos especiales a las instancias de este modelo (como createOtroModelo(), getOtroModelos()).

belongsTo(OtroModelo): Método que se usa en el modelo "muchos". Establece que una instancia de este modelo pertenece a una instancia del otro.

Clave Foránea (Foreign Key): La columna que se crea automáticamente (ej: UsuarioId en la tabla Publicaciones) para mantener el vínculo entre las tablas. Sequelize gestiona esto por ti.

Carga Ansiosa (Eager Loading): La técnica de obtener datos de modelos relacionados en una sola consulta, utilizando la opción include en los métodos de búsqueda (findAll, findByPk).
